...
import { AuthenticationService, FidoError } from './authentication.service';
...
export class AppLifecycleService {
  ...
  private async handleAppResume(): Promise<void> {
    try {
      ...
      // Handle FIDO2 errors specifically
      if (error instanceof FidoError) {
        console.log('FIDO2 authentication fallback requested');
        return;
      }

      // Only redirect to login for non-FIDO2 errors
      if (!(error instanceof FidoError)) {
        await this.router.navigate(['/login']);
      }
    }
