...
import { CRUD_OPERATIONS, DATA_TYPE, INPUT_OUTPUT, STATUS, STORAGE } from '@app/core/constants';
...
export class TransactionsService {
   ...
   */
  private getInputOutputAction(inputOutput: string): string {
  ...
  private async listByActivity(idActividad: string): Promise<Transaccion[]> {
    try {
      ...
      const transacciones = transaction.Transacciones.filter(x => x.IdProceso === idActividad);
      ...
      return transacciones.map(transaccion => {
        ...
        transaccion.Accion = this.getInputOutputAction(operacion ?? '');
  ...
  async get(idActividad: string, idTransaccion: string): Promise<Transaccion | undefined> {
    try {
      ...
      const transaccion = transaction.Transacciones.find(
        x => x.IdProceso === idActividad && x.IdTransaccion === idTransaccion
      );
  ...
  async getByPoint(idActividad: string, idPunto: string): Promise<Transaccion | undefined> {
    try {
      ...
      return transaction.Transacciones.find(
        x => x.IdProceso === idActividad && x.IdDeposito === idPunto
      );
    } catch (error) {
  ...
  async getByThirdParty(idActividad: string, idTercero: string): Promise<Transaccion | undefined> {
    try {
      ...
      return transaction.Transacciones.find(
        x => x.IdProceso === idActividad && x.IdTercero === idTercero
      );
    } catch (error) {
    ...
    try {
      ...
      if (transaction) {
        const index = transaction.Transacciones.findIndex(
          x => x.IdProceso === transaccion.IdProceso && x.IdTransaccion === transaccion.IdTransaccion
        );
